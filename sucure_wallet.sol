...
contract SecureWallet is Ownable {
  ...
  function depositNative(uint256 amount) payable external onlyOwner{
    require(payable(msg.sender).balance >= amount, "Invalid amount");
  ...
  function withDrawNativeAllTo(address account) external onlyOwner{
  ...
  function withDrawNative(uint256 amount) external onlyOwner{
    require(payable(address(this)).balance >= amount, "Invalid amount");

    payable(msg.sender).transfer(amount);
  }

  function withDrawNativeTo(address account, uint256 amount) external onlyOwner{
    require(payable(address(this)).balance >= amount, "Invalid amount");
    require(account != address(0), "Invalid account");

    payable(msg.sender).transfer(account, amount);
  }
  ...
  function withdraw(uint256 amount, IERC20 token) external onlyOwner{
    require(token.balanceOf(address(this)) >= amount, "Invalid amount");

    token.transfer(msg.sender, amount);
  }

  function withdrawTo(address account, uint256 amount, IERC20 token) external onlyOwner{
    require(token.balanceOf(address(this)) >= amount, "Invalid amount");
    require(account != address(0), "Invalid account");
